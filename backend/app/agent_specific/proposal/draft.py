...
from app.agent_specific.proposal.fee_table import render_frequency_table, render_simple_table
from app.agent_specific.proposal.fee_table import (
...
from app.agent_specific.proposal.loaders import (
    load_template_yaml,
    read_static_block,
)
...
def _refresh_draft_fee_row_displays(draft: dict[str, Any]) -> dict[str, Any]:
    from app.agent_specific.proposal.fee_row import iter_fee_rows
...
def add_package_to_draft(draft: dict[str, Any], package: dict[str, Any], services: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if existing_table is not None:
        ...
        if existing_table.get("brief") is None:
            ...
            from app.agent_specific.proposal.placeholders import build_table_brief
    ...
    template_id = str((updated.get("meta") or {}).get("template_id") or "").strip()
    from app.agent_specific.proposal.placeholders import build_table_brief, sync_draft_template_placeholders
...
def render_draft_markdown(draft: dict[str, Any]) -> str:
    ...
    for section in _sections(draft):
        ...
        if kind == "static_block":
            ...
            from app.agent_specific.proposal.placeholders import resolve_section_source_content
...
def _render_fee_section(draft: dict[str, Any], section: dict[str, Any]) -> str:
    ...
    from app.agent_specific.proposal.footnotes import apply_footnote_numbers, collect_footnotes, render_footnotes_footer
    ...
    if intro:
        parts.extend([intro, ""])
    template_id = str((draft.get("meta") or {}).get("template_id") or "")
    from app.agent_specific.proposal.placeholders import render_fee_section_briefs
...
def build_draft_preview(draft: dict[str, Any]) -> dict[str, Any]:
    ...
    from app.agent_specific.proposal.export_service import word_export_status
