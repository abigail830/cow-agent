...
from app.platform.hooks.hook_catalog import merge_hook_params

# Stable middleware order within FunctionMiddleware layer (sql_viz must run last).
_HOOK_ORDER: dict[str, int] = {
    "sql_validator": 10,
    "result_truncator": 20,
    "proposal_persist": 30,
    "sql_viz": 90,
...
def _sort_hook_entries(entries: list[tuple[str, dict[str, Any]]]) -> list[tuple[str, dict[str, Any]]]:
    return sorted(entries, key=lambda item: (_HOOK_ORDER.get(item[0], 50), item[0]))
...
def normalize_hooks(
    ...
    *,
    ...
    if isinstance(hooks_raw, list):
        ...
        return _sort_hook_entries(ordered)
