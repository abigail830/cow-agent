...
@lru_cache
def _limits_from_settings() -> tuple[int, int, int, int, int]:
    ...
    return (
        ...
        settings.attachment_max_pages_per_file,
        settings.attachment_max_pages_per_message,
    )


def attachment_limits(settings: Settings | None = None) -> tuple[int, int, int, int, int]:
    if settings is None:
...
def attachment_limits_dict(settings: Settings | None = None) -> dict[str, int]:
    max_files, max_file_bytes, max_total_bytes, max_pages_file, max_pages_message = attachment_limits(
        settings
    )
    return {
        ...
        "max_pages_per_file": max_pages_file,
        "max_pages_per_message": max_pages_message,
    }
