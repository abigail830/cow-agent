...
NOTION_RESOURCE = "https://mcp.notion.com"
NOTION_AUTHORIZE_URL = "https://mcp.notion.com/authorize"
...
class NotionIntegrationProvider:
    ...
    def is_platform_configured(self) -> bool:
        return bool(
            self._settings.notion_mcp_client_id
            and self._settings.notion_mcp_client_secret
            and self._settings.notion_mcp_redirect_uri
        )

    def resource(self) -> str:
        return NOTION_RESOURCE

    def build_authorize_url(self, *, state: str, code_challenge: str) -> str:
        client_id = self._settings.notion_mcp_client_id
        redirect_uri = self._settings.notion_mcp_redirect_uri
        if not client_id or not redirect_uri:
            raise ValueError("Notion MCP OAuth is not configured")
        ...
        body = await self._post_token(
            {
                ...
                "code": code,
                "redirect_uri": self._settings.notion_mcp_redirect_uri or "",
    ...
    async def _post_token(
        ...
        *,
        ...
        client_id = self._settings.notion_mcp_client_id
        client_secret = self._settings.notion_mcp_client_secret
        ...
        if not response.is_success:
            ...
            raise ValueError(f"Notion token request failed ({response.status_code}): {detail}")
        ...
        if require_refresh_token and not data.get("refresh_token"):
            raise ValueError("Notion token response missing refresh_token")
    ...
    def _bundle_from_response(
        ...
        *,
        ...
        scope_raw = body.get("scope") or body.get("scopes") or ""
        scopes = [part.strip() for part in str(scope_raw).split() if part.strip()]
        metadata: dict[str, Any] = {}
        for key in ("workspace_id", "workspace_name", "bot_id", "owner"):
            if key in body:
        ...
        if isinstance(workspace_name, str) and workspace_name.strip():
            account_label = workspace_name.strip()
