...
| 层 | 位置 | 内容 |
|----|------|------|
| **角色与对外表述** | `agents/au-sp-analysis/system_prompt.md` | 读者画像、业务语言、PII、禁止泄露实现细节 |
| **运行时** | `agents/au-sp-analysis/profile.yaml` | 只读 MCP、**必须加载本 Skill** |
...
service_and_fee_au_incorp ── catalog（SKU / 标准定价，对照 state 与 deal）
```
...
| **service_and_fee_au_incorp** | 产品目录 | `sku`, `service_name`, `is_package`, `hubspot_price`, `standard_pricing`, `is_active`, `proposal_types` |
...
| SKU 对照目录 | `JSON` 中 `sku` ↔ `service_and_fee_au_incorp.sku` |
...
| 标准价 / 套餐定义 | **`service_and_fee_au_incorp`** |
...
| 路径 | 含义 |
|------|------|
| `proposal_type` | 报价类型；AU 常为 `incorp_au_advisory` / `incorp_au_audit` |
| `stage` | 当前流程阶段（字符串，可空） |
...
- `proposal_type IN ('incorp_au_advisory', 'incorp_au_audit')` 除非用户指定其他类型
...
- 目录对照：`service_and_fee_au_incorp.is_active IS TRUE`
...
### 分析口径（可选，1–3 句白话）
- 例：「统计过去 30 天非模板的 AU ADVISORY 报价会话」「已排除内部用户」「按最新报价内容统计服务出现次数」
...
| `service_and_fee_au_incorp` | 标准服务目录与标价 |
