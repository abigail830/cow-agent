...
from app.agent_specific.yl_worker2.triggers.handler import handle_yl_worker2_trigger
...
async def test_handle_trigger_auto_run_false_persists_only():
    ...
    with (
        patch("app.agent_specific.yl_worker2.triggers.handler._persist_trigger_user_message", new_callable=AsyncMock) as persist,
        patch("app.agent_specific.yl_worker2.triggers.handler.ChatRunService") as run_cls,
    ):
...
async def test_handle_trigger_auto_run_true_runs_agent():
    ...
    with patch("app.agent_specific.yl_worker2.triggers.handler.ChatRunService") as run_cls:
