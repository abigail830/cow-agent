...
| `/home/user/content-studio/skills/pptx/references` | `pptxgenjs.md`, `ascentium-deck.md`, `inspire-deck.md`, `lrqa-deck.md` |
| `/home/user/content-studio/skills/pptx/assets` | `ascentium/*.png`, `inspire/*.png`, `lrqa/*.png` (brand chrome) |
| `/home/user/content-studio/skills/html-slides/references` | `ascentium-deck.md`, `inspire-deck.md`, `lrqa-deck.md` |
| `/home/user/content-studio/skills/html-slides/assets` | `ascentium/*.png`, `inspire/*.png`, `lrqa/*.png` (brand chrome) |
...
cd backend
export E2B_API_KEY=...
npm install   # once — installs e2b + tsx
...
Copy source paths are under `agents/content-studio/skills/` (relative to `backend/` as `fileContextPath`).
...
sandbox:
  provider: e2b
  templateId: okf-content-studio:1.15   # alias:tag — bare alias also resolves via platform default tag
...
pdftoppm -v
pandoc --version
ls /home/user/content-studio/skills/pptx/assets/lrqa/lrqa_logo.png
```
...
4. Update `E2B_CONTENT_STUDIO_TEMPLATE` in backend `.env` / deploy env to match.
5. Redeploy backend — no code change needed if alias name unchanged.

**1.15** — LRQA theme (`lrqa-deck.md` + `assets/lrqa/`); copy source fixed to `agents/content-studio/skills/`.
